...
"""
    ...
    def _heuristic_generate(self, desc: str, domain: str, catalog: str) -> str:
        ...
        if source_type == "kafka":
            ...
            lines.append("    secret_scope: kafka-prod")
            lines.append("    starting_offsets: latest")
        elif source_type == "auto_loader":
            lines.append(f"    path: /Volumes/{catalog}/raw/{table_name}/")
            lines.append("    format: json")
        elif source_type == "jdbc":
            lines.append("    secret_scope: jdbc-prod")
        ...
        if classification == "mnpi":
            lines.append("  embargo_hours: 24")

        # Schedule
        if mode == "streaming":
            lines.extend(["", "schedule:", '  cron: "*/5 * * * *"', "  sla_minutes: 10"])
        else:
            lines.extend(["", "schedule:", '  cron: "0 6 * * *"', "  sla_minutes: 60"])

        if "alert" in desc_lower or "notify" in desc_lower:
            lines.extend(["", "alerting:", "  on_failure: [slack:#data-alerts]"])
