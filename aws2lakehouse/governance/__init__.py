...
class UnityCatalogSetup:
    ...
    def generate_setup_sql(self) -> str:
        ...
        for env in self.environments:
            ...
            sql_parts.append(f"  COMMENT 'Shared utilities and reference data';")
            sql_parts.append(f"CREATE SCHEMA IF NOT EXISTS {catalog_name}.external")
            sql_parts.append(f"  COMMENT 'External volumes for cloud storage access';")
            sql_parts.append(f"CREATE SCHEMA IF NOT EXISTS {catalog_name}.audit")
            sql_parts.append(f"  COMMENT 'Audit logs and compliance tracking';\n")
    ...
    def generate_access_policies(self) -> List[AccessPolicy]:
        ...
        for env in self.environments:
            ...
            policies.append(AccessPolicy(
                ...
                securable_name=catalog if env == "dev" else ""
            ))
...
""")
            stmts.append(f"-- Grant access to view (restrict direct table access)")
