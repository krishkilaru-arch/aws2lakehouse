-- Monitoring: partner_data_sync
-- Freshness SLA check
SELECT 'partner_data_sync' as pipeline, max(_ingested_at) as last_ingest,
...
FROM production.finance_bronze.partner_data_sync;
...
SELECT date(_ingested_at) as dt, count(*) as row_count,
  ...
  CASE
    ...
    ELSE 'NORMAL'
  END as status
FROM production.finance_bronze.partner_data_sync GROUP BY 1 ORDER BY 1 DESC LIMIT 14;
