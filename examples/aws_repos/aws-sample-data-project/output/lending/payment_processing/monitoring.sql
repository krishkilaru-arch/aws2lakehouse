-- Monitoring: payment_processing
-- Freshness SLA check
SELECT 'payment_processing' as pipeline, max(_ingested_at) as last_ingest,
  current_timestamp() - max(_ingested_at) as staleness,
  CASE WHEN current_timestamp() - max(_ingested_at) > INTERVAL 10 MINUTES THEN 'BREACH' ELSE 'OK' END as sla_status
FROM production.lending_bronze.payment_processing;
...
SELECT date(_ingested_at) as dt, count(*) as row_count,
  ...
  CASE
    ...
    ELSE 'NORMAL'
  END as status
FROM production.lending_bronze.payment_processing GROUP BY 1 ORDER BY 1 DESC LIMIT 14;
