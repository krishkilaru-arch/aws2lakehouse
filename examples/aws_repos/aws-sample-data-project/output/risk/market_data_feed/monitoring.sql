-- Monitoring: market_data_feed
-- Freshness SLA check
SELECT 'market_data_feed' as pipeline, max(_ingested_at) as last_ingest,
  current_timestamp() - max(_ingested_at) as staleness,
  CASE WHEN current_timestamp() - max(_ingested_at) > INTERVAL 2 MINUTES THEN 'BREACH' ELSE 'OK' END as sla_status
FROM production.risk_bronze.market_data_feed;
...
SELECT date(_ingested_at) as dt, count(*) as row_count,
  ...
  CASE
    ...
    ELSE 'NORMAL'
  END as status
FROM production.risk_bronze.market_data_feed GROUP BY 1 ORDER BY 1 DESC LIMIT 14;
