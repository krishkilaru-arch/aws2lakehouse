-- Monitoring: daily_risk_aggregation
-- Freshness SLA check
SELECT 'daily_risk_aggregation' as pipeline, max(_ingested_at) as last_ingest,
  current_timestamp() - max(_ingested_at) as staleness,
  CASE WHEN current_timestamp() - max(_ingested_at) > INTERVAL 90 MINUTES THEN 'BREACH' ELSE 'OK' END as sla_status
FROM production.risk_bronze.daily_risk_aggregation;
...
SELECT date(_ingested_at) as dt, count(*) as row_count,
  ...
  CASE
    ...
    ELSE 'NORMAL'
  END as status
FROM production.risk_bronze.daily_risk_aggregation GROUP BY 1 ORDER BY 1 DESC LIMIT 14;
