-- Monitoring: compliance_report
-- Freshness SLA check
SELECT 'compliance_report' as pipeline, max(_ingested_at) as last_ingest,
  current_timestamp() - max(_ingested_at) as staleness,
  CASE WHEN current_timestamp() - max(_ingested_at) > INTERVAL 180 MINUTES THEN 'BREACH' ELSE 'OK' END as sla_status
FROM production.compliance_bronze.compliance_report;
...
SELECT date(_ingested_at) as dt, count(*) as row_count,
  ...
  CASE
    ...
    ELSE 'NORMAL'
  END as status
FROM production.compliance_bronze.compliance_report GROUP BY 1 ORDER BY 1 DESC LIMIT 14;
