...
SELECT 'customer_360' as pipeline, max(_ingested_at) as last_ingest,
  ...
  CASE WHEN current_timestamp() - max(_ingested_at) > INTERVAL 120 MINUTES THEN 'BREACH' ELSE 'OK' END as sla_status
FROM production.customer_bronze.customer_360;
...
SELECT date(_ingested_at) as dt, count(*) as row_count,
  ...
  CASE
    ...
    ELSE 'NORMAL'
  END as status
FROM production.customer_bronze.customer_360 GROUP BY 1 ORDER BY 1 DESC LIMIT 14;
