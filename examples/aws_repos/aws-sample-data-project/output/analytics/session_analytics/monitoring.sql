-- Monitoring: session_analytics
-- Freshness SLA check
SELECT 'session_analytics' as pipeline, max(_ingested_at) as last_ingest,
  current_timestamp() - max(_ingested_at) as staleness,
  CASE WHEN current_timestamp() - max(_ingested_at) > INTERVAL 45 MINUTES THEN 'BREACH' ELSE 'OK' END as sla_status
FROM production.analytics_bronze.session_analytics;
...
SELECT date(_ingested_at) as dt, count(*) as row_count,
  ...
  CASE
    ...
    ELSE 'NORMAL'
  END as status
FROM production.analytics_bronze.session_analytics GROUP BY 1 ORDER BY 1 DESC LIMIT 14;
