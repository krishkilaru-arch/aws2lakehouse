-- Monitoring: silver_to_gold
-- Freshness SLA check
SELECT 'silver_to_gold' as pipeline, max(_ingested_at) as last_ingest,
...
FROM acme_prod.finance_bronze.silver_to_gold;
...
SELECT date(_ingested_at) as dt, count(*) as row_count,
  ...
  CASE
    ...
    ELSE 'NORMAL'
  END as status
FROM acme_prod.finance_bronze.silver_to_gold GROUP BY 1 ORDER BY 1 DESC LIMIT 14;
