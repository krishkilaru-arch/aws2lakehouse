-- Monitoring: gold_aggregation
-- Freshness SLA check
SELECT 'gold_aggregation' as pipeline, max(_ingested_at) as last_ingest,
...
FROM acme_prod.finance_bronze.gold_aggregation;
...
SELECT date(_ingested_at) as dt, count(*) as row_count,
  ...
  CASE
    ...
    ELSE 'NORMAL'
  END as status
FROM acme_prod.finance_bronze.gold_aggregation GROUP BY 1 ORDER BY 1 DESC LIMIT 14;
