-- Monitoring: trade_events_streaming
-- Freshness SLA check
SELECT 'trade_events_streaming' as pipeline, max(_ingested_at) as last_ingest,
...
FROM acme_prod.risk_bronze.trade_events_streaming;
...
SELECT date(_ingested_at) as dt, count(*) as row_count,
  ...
  CASE
    ...
    ELSE 'NORMAL'
  END as status
FROM acme_prod.risk_bronze.trade_events_streaming GROUP BY 1 ORDER BY 1 DESC LIMIT 14;
