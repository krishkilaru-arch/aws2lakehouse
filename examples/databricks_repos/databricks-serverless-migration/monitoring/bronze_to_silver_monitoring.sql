-- Monitoring: bronze_to_silver
-- Freshness SLA check
SELECT 'bronze_to_silver' as pipeline, max(_ingested_at) as last_ingest,
...
FROM acme_prod.finance_bronze.bronze_to_silver;
...
SELECT date(_ingested_at) as dt, count(*) as row_count,
  ...
  CASE
    ...
    ELSE 'NORMAL'
  END as status
FROM acme_prod.finance_bronze.bronze_to_silver GROUP BY 1 ORDER BY 1 DESC LIMIT 14;
