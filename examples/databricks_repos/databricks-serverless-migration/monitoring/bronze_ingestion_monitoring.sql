-- Monitoring: bronze_ingestion
-- Freshness SLA check
SELECT 'bronze_ingestion' as pipeline, max(_ingested_at) as last_ingest,
...
SELECT date(_ingested_at) as dt, count(*) as row_count,
  ...
  CASE
    ...
    ELSE 'NORMAL'
  END as status
FROM acme_prod.finance_bronze.bronze_ingestion GROUP BY 1 ORDER BY 1 DESC LIMIT 14;
