-- Monitoring: feature_store_builder
-- Freshness SLA check
SELECT 'feature_store_builder' as pipeline, max(_ingested_at) as last_ingest,
...
FROM acme_prod.finance_bronze.feature_store_builder;
...
SELECT date(_ingested_at) as dt, count(*) as row_count,
  ...
  CASE
    ...
    ELSE 'NORMAL'
  END as status
FROM acme_prod.finance_bronze.feature_store_builder GROUP BY 1 ORDER BY 1 DESC LIMIT 14;
