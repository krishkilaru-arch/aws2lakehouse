-- Monitoring: risk_aggregation_job
-- Freshness SLA check
SELECT 'risk_aggregation_job' as pipeline, max(_ingested_at) as last_ingest,
...
FROM acme_prod.finance_bronze.risk_aggregation_job;
...
SELECT date(_ingested_at) as dt, count(*) as row_count,
  ...
  CASE
    ...
    ELSE 'NORMAL'
  END as status
FROM acme_prod.finance_bronze.risk_aggregation_job GROUP BY 1 ORDER BY 1 DESC LIMIT 14;
