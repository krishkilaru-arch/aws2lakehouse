-- Monitoring: nav_calculation_job
-- Freshness SLA check
SELECT 'nav_calculation_job' as pipeline, max(_ingested_at) as last_ingest,
...
FROM acme_prod.finance_bronze.nav_calculation_job;
...
SELECT date(_ingested_at) as dt, count(*) as row_count,
  ...
  CASE
    ...
    ELSE 'NORMAL'
  END as status
FROM acme_prod.finance_bronze.nav_calculation_job GROUP BY 1 ORDER BY 1 DESC LIMIT 14;
