-- Monitoring: order_events_processing
-- Freshness SLA check
SELECT 'order_events_processing' as pipeline, max(_ingested_at) as last_ingest,
...
FROM acme_prod.finance_bronze.order_events_processing;
...
SELECT date(_ingested_at) as dt, count(*) as row_count,
  ...
  CASE
    ...
    ELSE 'NORMAL'
  END as status
FROM acme_prod.finance_bronze.order_events_processing GROUP BY 1 ORDER BY 1 DESC LIMIT 14;
