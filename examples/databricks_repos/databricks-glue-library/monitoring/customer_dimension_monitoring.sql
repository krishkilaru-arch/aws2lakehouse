-- Monitoring: customer_dimension
-- Freshness SLA check
SELECT 'customer_dimension' as pipeline, max(_ingested_at) as last_ingest,
...
FROM acme_prod.finance_bronze.customer_dimension;
...
SELECT date(_ingested_at) as dt, count(*) as row_count,
  ...
  CASE
    ...
    ELSE 'NORMAL'
  END as status
FROM acme_prod.finance_bronze.customer_dimension GROUP BY 1 ORDER BY 1 DESC LIMIT 14;
