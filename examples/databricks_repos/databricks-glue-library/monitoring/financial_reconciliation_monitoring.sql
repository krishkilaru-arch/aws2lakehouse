-- Monitoring: financial_reconciliation
-- Freshness SLA check
SELECT 'financial_reconciliation' as pipeline, max(_ingested_at) as last_ingest,
...
FROM acme_prod.finance_bronze.financial_reconciliation;
...
SELECT date(_ingested_at) as dt, count(*) as row_count,
  ...
  CASE
    ...
    ELSE 'NORMAL'
  END as status
FROM acme_prod.finance_bronze.financial_reconciliation GROUP BY 1 ORDER BY 1 DESC LIMIT 14;
